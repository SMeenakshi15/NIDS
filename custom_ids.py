...
while True:
        ...
        while True:
            ...
            if event == '-pkts-' and len(values['-pkts-']):     # if a list item is chosen
                ...
                try:
                    ...
                    pass
                #window['-payloaddecoded-'].update(value=sus_readablepayloads[sus_selected_index ]) ## changed
                window['-payloaddecoded-'].update(value=sus_readablepayloads[int(sus_selected_index)])
            ...
            if event == "-http2streams-":
                ...
                http2streamindex = values.get(event, [])[0]
    ...
    if event == '-savepcap-':
        pcapname = "nettrafic"
        #scp.wrpcap(f'.\\savedpcap\\{pcapname}.pcap', inc_pkt_list)  ##change done
        scp.wrpcap(f'.\\temp\\{pcapname}.pcap', inc_pkt_list)
    if event == '-pkts-' and len(values['-pkts-']):     # if a list item is chosen
        sus_selected = values['-pkts-']
        #sus_selected_index = int(sus_selected.split()[0][0:2])
